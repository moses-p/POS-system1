"""
Script to replace specific functions in app.py with fixed versions
"""
...
def replace_functions():
    ...
    with open('app.py.full_backup', 'w', encoding='utf-8') as file:
        file.write(content)
    
    # Replace create_order function
    pattern = r'def create_order\([^)]*\):[^@]*?(?=@app\.|\ndef |\Z)'
    content = re.sub(pattern, CREATE_ORDER_FUNCTION, content, flags=re.DOTALL)
    
    # Replace catch_all function
    pattern = r'@app\.route\(\'/(?:<path:path>)\'\)[^@]*?(?=@app\.|\ndef |\Z)'
    content = re.sub(pattern, CATCH_ALL_FUNCTION, content, flags=re.DOTALL)
    
    # Replace get_order_by_id function
    pattern = r'def get_order_by_id\([^)]*\):[^@]*?(?=@app\.|\ndef |\Z)'
    content = re.sub(pattern, GET_ORDER_BY_ID_FUNCTION, content, flags=re.DOTALL)
    
    with open('app_fixed_direct.py', 'w', encoding='utf-8') as file:
        file.write(content)
    
    print("Direct function replacement completed. Output saved to app_fixed_direct.py")
