...
def get_order_by_id(order_id):
    ...
    try:
        ...
        class SimpleOrder:
            def __init__(self, order_dict, items_list):
                self.id = order_dict['id']
                self.reference_number = order_dict['reference_number']
                self.customer_name = order_dict['customer_name']
                self.order_date = order_dict['order_date']
                self.total_amount = order_dict['total_amount']
            ...
            def items(self):
                ...
                class SimpleOrderItem:
                    def __init__(self, item_dict):
                        ...
                        self.product_id = item_dict['product_id']
        ...
        logger.warning(f"SQLAlchemy error when retrieving order {order_id}, falling back to direct SQL: {str(e)}")
        
        try:
            # Fall back to direct SQL
            conn = sqlite3.connect('instance/pos.db')
            conn.row_factory = sqlite3.Row  # This makes the rows behave more like dictionaries
            cursor = conn.cursor()
            ...
            # Create a simple object with the essential info
            class SimpleOrder:
                def __init__(self, id, reference_number, customer_name, order_date, total_amount, status, items=None):
                    self.id = id
            ...
            # Add items if needed (this is simplified)
            # For a complete implementation, you would need to create SimpleOrderItem objects
            
            return order
            
        except Exception as e:
            logger.error(f"Error retrieving order {order_id} with direct SQL: {str(e)}")
            return None
