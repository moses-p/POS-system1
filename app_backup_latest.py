...
def checkout():
    try:
        ...
        try:
            # Check if we have a last_order_id in session to prevent duplicate submissions
            last_order_id = session.get('last_order_id')
            last_order_timestamp = session.get('last_order_timestamp')
            current_time = datetime.utcnow()
            
            # If we processed an order in the last 10 seconds, likely a duplicate submission
            if last_order_id and last_order_timestamp:
                try:
                    last_time = datetime.fromisoformat(last_order_timestamp)
                    time_diff = (current_time - last_time).total_seconds()
                    if time_diff < 10:  # Within 10 seconds
                        logger.warning(f"Possible duplicate order submission: {last_order_id}")
                        flash("Your order was already processed!", "info")
                        return redirect(url_for('print_receipt', order_id=last_order_id))
                    else:
                        logger.info(f"Order submission time: {time_diff} seconds")
                except (ValueError, TypeError):
                    # If timestamp conversion fails, continue normal processing
                    pass
            ...
            session['last_order_timestamp'] = datetime.utcnow().isoformat()
...
def print_receipt(order_id):
    try:
        ...
        return redirect(url_for('index'))
def get_currencies():
    """Get a list of currencies for dropdowns"""
    return [
...
def staff_dashboard():
    try:
        ...
        try:
            # Use raw SQL instead of ORM to avoid issues with columns
            conn = db.engine.raw_connection()
            cursor = conn.cursor()
            
            cursor.execute("""
                SELECT id, customer_name, total_amount, order_date, status, viewed
                FROM "order" 
            ...
            # Get pending orders specifically for the dashboard
            cursor.execute("""
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM 'order' WHERE status = 'processing'")
            processing = cursor.fetchone()[0] or 0
            
            cursor.execute("SELECT COUNT(*) FROM 'order' WHERE status = 'completed'")
...
def staff_order_detail(order_id):
    """Show details of a specific order for staff users"""
    try:
        order = get_order_by_id(order_id)
        if not order:
            flash('Order not found', 'error')
        ...
        # Make sure the staff_update_order_status route is properly defined
        app.jinja_env.globals['staff_update_order_status_url'] = url_for('staff_update_order_status', order_id=order_id)
...
def create_order(customer_data, items_data, order_type):
    ...
    try:
        ...
        try:
            ...
            # Check for potentially duplicate orders in the last minute
            ...
            cursor.execute("""
                ...
                ORDER BY order_date DESC
                LIMIT 1
            """, (customer_name, customer_phone, total_amount, one_minute_ago))
            
            potential_duplicate = cursor.fetchone()
            
            if potential_duplicate:
                order_id, reference_number = potential_duplicate
                
                # Get order items to confirm it's really a duplicate
                cursor.execute("""
                    SELECT product_id FROM order_item WHERE order_id = ?
                """, (order_id,))
                
                existing_product_ids = sorted([row[0] for row in cursor.fetchall()])
                existing_product_ids_str = ','.join(str(pid) for pid in existing_product_ids)
                
                # If the products match, it's likely a duplicate
                if product_ids_str == existing_product_ids_str:
                    logger.warning(f"Detected duplicate order attempt - using existing order {order_id}")
                    order = get_order_by_id(order_id)
                    conn.close()
                    return order, "Possible duplicate order - using existing order"
...
def get_order_by_id(order_id):
    ...
    try:
        ...
        try:
            from datetime import datetime
            if order_dict.get('order_date') and isinstance(order_dict['order_date'], str):
                try:
            ...
            if order_dict.get('updated_at') and isinstance(order_dict['updated_at'], str):
                try:
                    order_dict['updated_at'] = datetime.strptime(order_dict['updated_at'], '%Y-%m-%d %H:%M:%S')
                except (ValueError, TypeError):
                    # Leave it as string if conversion fails
                    pass
                    
            if order_dict.get('completed_at') and isinstance(order_dict['completed_at'], str):
                try:
                    order_dict['completed_at'] = datetime.strptime(order_dict['completed_at'], '%Y-%m-%d %H:%M:%S')
                except (ValueError, TypeError):
                    # Leave it as string if conversion fails
                    pass
                    
            if order_dict.get('viewed_at') and isinstance(order_dict['viewed_at'], str):
                try:
                    order_dict['viewed_at'] = datetime.strptime(order_dict['viewed_at'], '%Y-%m-%d %H:%M:%S')
                except (ValueError, TypeError):
                    # Leave it as string if conversion fails
                    pass
        except Exception as date_err:
            logger.warning(f"Error converting dates: {str(date_err)}")
            # Continue with string dates
...
def staff_update_order_status(order_id):
    ...
    try:
        ...
        # Get order using our direct SQL method
        order = get_order_by_id(order_id)
        if not order:
            flash('Order not found', 'error')
            return redirect(url_for('staff_dashboard'))
...
def in_store_sale():
    ...
    # Allow both admin and staff to create in-store sales
    ...
    if request.method == 'POST':
        try:
            # Check if we have a last_order_id in session to prevent duplicate submissions
            last_order_id = session.get('last_store_order_id')
            last_order_timestamp = session.get('last_store_order_timestamp')
            current_time = datetime.utcnow()
            ...
            if last_order_id and last_order_timestamp:
                try:
                    ...
                    time_diff = (current_time - last_time).total_seconds()
                    if time_diff < 10:  # Within 10 seconds
                        logger.warning(f"Possible duplicate in-store sale submission: {last_order_id}")
                        flash("Your order was already processed!", "info")
                        return redirect(url_for('print_receipt', order_id=last_order_id))
                    else:
                        logger.info(f"Order submission time: {time_diff} seconds")
            ...
            # Store the order ID in session to prevent duplicates
            session['last_store_order_id'] = order.id
            session['last_store_order_timestamp'] = datetime.utcnow().isoformat()
...
def check_new_orders():
    try:
        # Allow access for both admin and staff roles
        # We're not enforcing staff_required decorator to allow both admin and staff interfaces
        # to receive order notifications
        if not current_user.is_authenticated:
            # Return empty result for non-authenticated users
